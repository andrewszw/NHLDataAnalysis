...
def scrape_season(url, team_name, year):
    ...
    for row in table.find_all('tr')[1:]:    
        ...
        if cells[2].find_all('li')[0].span.text == 'L':
            team_score = int(score[1])
            opponent_team_score = int(score[0])
        elif cells[2].find_all('li')[0].span.text == 'W':
            team_score = int(score[0])
            opponent_team_score = int(score[1])
        ...
        game_list.append([
            team_name, opponent_team, home_team, team_score, opponent_team_score,
            shots_for, shots_against, powerplay_success,
            powerplay_attempts, penalty_kill_success, penalty_kill_attempts, game_date, year
        ])
        """game_list.append([
